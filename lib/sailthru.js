(function() {
  ...
  SailthruRequest = (function() {
    ...
    SailthruRequest.prototype._http_request = function(uri, data, method, binary_data_params, callback) {
      return new Promise((resolve, reject) => {
        ...
        // If only four arguments are passed, its not a multipart upload
        // This check sees if fourth argument is actually callback
        // Below shifts fourth argument to fifth, sets binary_data_parms to empty array

        // New code for promises callback
        if (typeof binary_data_params === 'undefined') {
          callback = function(err, res) { return err ? err : res };
          binary_data_params = undefined;
        // Old code... "else if" is just original "if"
        ...
        req = http_protocol.request(options, (function(res) {
          ...
          return res.on('end', function() {
            ...
            try {
              ...
              if (statusCode === 200) {
                resolve(callback(null, json_response))
              } else {
                json_err = {
                ...
                reject(callback(json_err, json_response));
              }
            } catch (_error) {
              error = _error;
              json_err = {
              ...
              reject(callback(json_err, error.message));
            }
          });
        }).bind(this));
        req.on('error', function(err) {
          reject(callback(err, err.message));
        });
        if (method === 'POST') {
    ...
    SailthruRequest.prototype._api_request = function(uri, data, request_method, binary_data_params, callback) {
      ...
      .then(res => {
        if (typeof binary_data_params !== 'undefined') return res;
        Promise.resolve(res)
      })
  ...
  SailthruClient = (function() {
    ...
    SailthruClient.prototype._apiRequest = async function(action, data, method, callback) {
      var _url, json_payload;
      _url = url.parse(this.api_url);
      json_payload = this._json_payload(data);
      // const _apiRequestResponse = this.request._api_request(_url.href + action, json_payload, method, callback);
      // console.log("API Request Response: ", _apiRequestResponse);
      // return _apiRequestResponse;
    ...
    SailthruClient.prototype.apiGet = async function(action, data, callback) {
      // const apiGetResponse = this._apiRequest(action, data, 'GET', callback);
      // console.log("API GET Response:", apiGetResponse);
